...
public class UIUtil {
    ...
    // draws the given statement centered at coordinates (pos_x, pos_y) and returns the width of the text
    public static void drawText(GraphicsContext gc, String statement, double pos_x, double pos_y) {
	Text text = new Text(statement);
	text.setFont(font);
	double width = text.getLayoutBounds().getWidth();
	double height = text.getLayoutBounds().getHeight();

        gc.strokeText(statement, pos_x - width / 2, pos_y);
	gc.setFont(font);
	System.out.println(statement + " (" + width + ", " + height + ")");
    }
	...
	// gc.strokeText("Start", pos_x + 0.25 * scale_x, pos_y + 0.75 * scale_y);
	drawText(gc, "Start", pos_x + scale_x / 2, pos_y + scale_y / 2);
    }
	...
	gc.strokeText(statement, pos_x + 0.25 * scale_x, pos_y + 0.75 * scale_y);
    }
